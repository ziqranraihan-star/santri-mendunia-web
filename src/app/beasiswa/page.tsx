...
interface ScholarshipItem { id: string; title: string; description: string; provider: string; category: string; level: string; country: string; region: string; deadline: string; registrationUrl: string; benefits: string[]; }
...
export default function BeasiswaPublicPage() {
  ...
  useEffect(() => { (async () => { const items = await getDocuments<ScholarshipItem>(COLLECTIONS.scholarships, [orderBy("createdAt", "desc")]); setData(items.filter((x: any) => x.isActive !== false)); setLoading(false); })(); }, []);
          ...
          return (
            <div key={s.id} className="bg-white rounded-xl border p-6 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center justify-between">
                ...
                {s.registrationUrl && <a href={s.registrationUrl} target="_blank" rel="noopener noreferrer"><Button size="sm" className="bg-teal hover:bg-teal-dark gap-1 text-xs"><ExternalLink className="w-3 h-3" /> Daftar</Button></a>}
              </div>
            </div>
          );
        })}</div>}
