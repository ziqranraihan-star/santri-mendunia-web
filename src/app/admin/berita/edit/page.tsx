...
function EditBeritaContent() {
  ...
  useEffect(() => {
    ...
    getDocument(COLLECTIONS.news, id).then((data: any) => {
      if (data) {
        setForm({
          title: data.title || "",
          ...
          editors: Array.isArray(data.editors) ? data.editors.join(", ") : "",
          relatedLinks: Array.isArray(data.relatedLinks) ? data.relatedLinks : Array.isArray(data.related_links) ? data.related_links : [],
        });
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateDocument(COLLECTIONS.news, id, {
        title: form.title,
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-teal-deep">Edit Berita</h1>
          <p className="text-sm text-muted-foreground">Perbarui artikel Santri News</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Link href="/admin/berita"><Button variant="outline" type="button">Batal</Button></Link>
